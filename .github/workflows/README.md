...
| --- | --- | --- |
| `tests.yml` | push/PR to `main`, manual | unittests on ubuntu/windows × py3.12–3.14 |
| `build-page-docs.yml` | `v*.*.*` tags, PR, manual | Sphinx docs build + deploy to GitHub Pages |
...
```sh
git mv .github/workflows/tests.yml.disabled .github/workflows/tests.yml
...
Then remove the leading `DISABLED` banner comment from each workflow file
(optional, files work either way) and restore the GitHub Actions badges in
`README.md`.
