...
/* ceil(log2(v)) for 0 < v <= UINT32_MAX (POSIX-only header; GCC/Clang intrinsic).
...
static inline size_t c_shm_block_ceil_log2(uint32_t v) {
    size_t floor = (size_t) (31u - __builtin_clz((unsigned int) v));
...
static inline size_t c_shm_block_bin(size_t capacity) {
...
static inline void c_shm_page_reclaim(shm_allocator* allocator, shm_page_ctx* page_ctx) {
    ...
    while (*prevp) {
        ...
        *prevp = block->next_allocated;
        ...
        shm_memory_block** free_prev = &allocator->bins[c_shm_block_bin(block->capacity)];
...
static inline void* c_shm_request(shm_allocator_ctx* ctx, size_t size, int scan_all_pages, pthread_mutex_t* lock) {
    ...
    size_t            bin = c_shm_block_bin(cap_net);
    shm_memory_block* free_blk;
    if (bin <= (size_t) AP_SHM_EXACT_BIN_COUNT) {
        ...
        shm_memory_block** prevp = &allocator->bins[bin];
        free_blk = *prevp;
        while (free_blk) {
...
static inline void c_shm_free(void* ptr, pthread_mutex_t* lock) {
    ...
    block->size = 0;
    size_t bin = c_shm_block_bin(block->capacity);
